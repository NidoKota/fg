...
namespace fg
{
    ...
    public:
            ...
            : resource_base(name, nullptr), description_(description), actual_(actual)
        {
            ...
            if (!actual)
            {
                // Retained resourceの場合、usage_infoは空の情報を渡す
                resource_usage_info usage_info{nullptr, &readers_, &writers_};
                actual_ = fg::realize<description_type, actual_type>(description_, usage_info);
            }
        }
    ...
    protected:
        void realize(const resource_usage_info &usage_info) override
        {
            if (transient())
                std::get<std::unique_ptr<actual_type>>(actual_) = fg::realize<description_type, actual_type>(description_, usage_info);
        }
